...
profile_filename = "C:/Users/sumayafathima.r/PycharmProjects/pythonProject2/ftd_details.yaml"
...
def get_token(host):
    ...
    request = requests.post(f"https://{host}:{FDM_PORT}/api/fdm/v{FDM_VERSION}/fdm/token",
                            json=payload, verify=False, headers=headers)
    if request.status_code == 400:
        raise Exception("Error logging in: {}".format(request.content))
    try:
        access_token = request.json()['access_token']
        fa = open("C:/Users/sumayafathima.r/PycharmProjects/pythonProject2/fdm_endpoints_token.txt", "w")
        fa.write(access_token)
        fa.close()
        new_auth_token[0] = access_token
        print(green("Token = " + access_token))
